...
import { useAuth0 } from "@auth0/auth0-react"; // import security functions
...
const DashboardPage = () => {
  ...
  const { getAccessTokenSilently } = useAuth0(); // Security Function
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setError(false);
        setErrorMessage("");
        const accessToken = await getAccessTokenSilently(); // Obtain security token
